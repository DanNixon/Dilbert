	...
	class Conways : public App{
		...
		void setFGBit(uint16_t x, uint16_t y, uint8_t val){
			...
			if(val){frontGrid[subX][y] = frontGrid[subX][y] | (1<<(x%8));}// turn bit on
			else{frontGrid[subX][y] = frontGrid[subX][y] & ~(1<<(x%8));}//turn bit off
		}
		...
		void setBGBit(uint16_t x, uint16_t y, uint16_t val){
			uint16_t subX=(float)x/8.0;
			if(val){backGrid[subX][y] = backGrid[subX][y] | (1<<(x%8));}// turn bit on
			else{backGrid[subX][y] = backGrid[subX][y] & ~(1<<(x%8));}//turn bit off
		}
		...
		bool cgolGetDot(uint16_t x, uint16_t y){
			...
			if(xleft<0)xleft=GRID_X-1;
			if(xright==GRID_X)xright=0;
			if(ytop==GRID_Y)ytop=0;
			if(ybottom<0)ybottom=GRID_Y-1;
			if(getFGBit(xleft,ytop))count++; // top left
			if(getFGBit(x,ytop))count++; // top middle
			if(getFGBit(xright,ytop))count++; // top right
			if(getFGBit(xleft,y))count++; // middle left
			if(getFGBit(xright,y))count++; // middle right
			if(getFGBit(xleft,ybottom))count++; // bottom left
			if(getFGBit(x,ybottom))count++; // bottom middle
			if(getFGBit(xright,ybottom))count++; // bottom right
			if(getFGBit(x,y) && count<2){ // < 2 && alive, death
				return false;
			}else if(getFGBit(x,y) && (count==2 || count==3)){ // ==2 || ==3 && alive, live
				return true;
			}else if(getFGBit(x,y) && count>3){ // >3 && alive, death
				return false;
			}else if(getFGBit(x,y)==false && count==3){ // ==3 && dead, birth
				return true;
			}
			return false;
		}
			...
			*/
			for(uint16_t x=0; x<GRID_X; x++){
				for(uint16_t y=0; y<GRID_Y; y++){
					...
					setFGBit(x,y,bit);
					m_badge->display().fillRect(x*PIXEL_SCALE,y*PIXEL_SCALE,PIXEL_SCALE,PIXEL_SCALE,bit?ILI9341_WHITE:ILI9341_BLACK);
				}
			}
			/*
			if(worldX<GRID_X && !getDots){
	...
	public:
		...
		bool handleButton(IButton *button){
			if(App::handleButton(button)) return true;
			
			if(button->isActive()){
				if(button->getID()==1){//up (next pattern)
					for(int x=0; x<GRID_X; x++){//clear grid
						for(int y=0; y<GRID_Y; y++){
							setFGBit(x,y,0);
						}
					}
					...
					for(int x=0; x<8; x++){//set pattern
						for(int y=0; y<8; y++){
							bool bit=getPattBit(currPattern,7-x,y);//kludge! invert x?!
							setFGBit(x+xOffset,y+yOffset,bit);
							m_badge->display().fillRect((x+xOffset)*PIXEL_SCALE,(y+yOffset)*PIXEL_SCALE,PIXEL_SCALE,PIXEL_SCALE,bit?ILI9341_WHITE:ILI9341_BLACK);
						}
					}
				}
				if(button->getID()==2){//down (prev pattern)
					for(int x=0; x<GRID_X; x++){//clear grid
						for(int y=0; y<GRID_Y; y++){
							setFGBit(x,y,0);
						}
				...
				if(button->getID()==0){//left: slower game speed
					wait+=100;
					wait=wait>10000?10000:wait;
				}
				if(button->getID()==3){//right: faster game speed
					wait-=100;
					wait=wait<100?100:wait;
				}
				if(button->getID()==4){//a (random pattern)
					//random fill screen pixels
					for(int x=0; x<GRID_X; x++){
						for(int y=0; y<GRID_Y; y++){
							bool bit=random(2);
